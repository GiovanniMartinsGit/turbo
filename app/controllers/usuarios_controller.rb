class UsuariosController < ApplicationController
  ...
  def index
    if params[:query].present?
      @usuarios = Usuario.search(params[:query])
    else
  ...
  def search
    @usuarios = Usuario.where('nome ILIKE ?', "%#{params[:query]}%")
    respond_to do |format|
      if turbo_frame_request?
        format.html { render partial: 'usuarios/datatable', locals: { usuarios: @usuarios } }
      else
  ...
  private
    ...
    def usuario_params
      params.require(:usuario).permit(:nome, :endereco)
    end
end
